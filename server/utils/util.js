...
const { CSRF_TOKEN_COOKIE_NAME, LOGIN_STATE_COOKIE_PREFIX, SESSION_COOKIE_NAME } = require('./constants');
...
exports.initSessionData = function (req, tokenData, userinfo) {
  // Set token data
  req.session.accessToken = tokenData.access_token;
  req.session.expiresAt = calculateExpTimeWithBuffer(tokenData.expires_in);
  req.session.refreshToken = tokenData.refresh_token;
  ...
  req.session.applicationId = userinfo.app_id;
