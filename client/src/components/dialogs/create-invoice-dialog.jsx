...
export function CreateInvoiceDialog({ fireConfetti, handleClose, open }) {
  return (
    <Dialog fullScreen={true} open={open} onClose={handleClose} TransitionComponent={Transition}>
      ...
      <Grid container maxWidth={1200} marginX="auto" sx={{ height: '100%' }}>
        <Grid container item xs={12} marginY="2rem">
          <Grid item xs={1} sm={2} />
          <Grid item xs={10} sm={8}>
            <DialogContent sx={{ height: '100%' }}>
              <CreateInvoiceForm fireConfetti={fireConfetti} closeFormDialog={handleClose} />
            </DialogContent>
          </Grid>
          <Grid item xs={1} sm={2} />
        </Grid>
